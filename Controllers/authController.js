...
var unsecureRoutes = ['/auth/login','/getClient','/trueCallback'];
...
var Auth = {
    ...
    createCookie: async function(req,res){
        ...
        try {
            ...
            jwt.sign({ id:res.locals.manager._id },config.secret,(err,token)=>{
    ...
    verifyUser : async function (req,res,next){
        ...
        let token = ( req.cookies.token )?(req.cookies.token):( req.header('token') );
    ...
    login : async function (req,res,next){
      ...
      try {
          ...
          var manager = await User.findOne({role:cat,number:number,password:pass});
